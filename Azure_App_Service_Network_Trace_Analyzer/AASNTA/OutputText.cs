...
namespace AASNTA
{
    ...
    class OutputText
    {
        ...
        static Workbook EXCEL_WORKBOOK = null;
        static Worksheet EXCEL_WORKSHEET = null;
        static string EXCEL_OUTPUT_FILEPATH = string.Empty;
        public static void TextReport(NetworkTrace Trace, string sourceIp, string destinationIp, string portNumber, Workbook excelWorkbook, Worksheet excelWorksheet, string excelOutputFilePath)
        {
            SOURCE_IP = sourceIp;
            ...
            EXCEL_OUTPUT_FILEPATH = excelOutputFilePath;
        ...
        private static void DisplayPossibleSQLServers(NetworkTrace Trace)
        {
            if (Trace.possibleSqlServers != null && Trace.possibleSqlServers.Count > 0)
            {
                ...
                Program.logMessage("If it is a valid SQL Server, copy the IP Address,Port into SQLNAUI and run the analysis again, or use /sql IP Address,Port in the AASNTA command-line if running directly.\r\n");
        ...
        private static void OutputStats(NetworkTrace Trace)
        {
            ...
            // Headers
            EXCEL_WORKSHEET.Cells[1, 1] = "SourceIP";
            EXCEL_WORKSHEET.Cells[1, 2] = "SourcePort";
            EXCEL_WORKSHEET.Cells[1, 3] = "DestIP";
            EXCEL_WORKSHEET.Cells[1, 4] = "DestPort";
            EXCEL_WORKSHEET.Cells[1, 5] = "IPVersion";
            EXCEL_WORKSHEET.Cells[1, 6] = "Protocol";
            EXCEL_WORKSHEET.Cells[1, 7] = "Syn";
            EXCEL_WORKSHEET.Cells[1, 8] = "Fin";
            EXCEL_WORKSHEET.Cells[1, 9] = "Reset";
            EXCEL_WORKSHEET.Cells[1, 10] = "AckSynDelayms";
            EXCEL_WORKSHEET.Cells[1, 11] = "Retransmit";
            EXCEL_WORKSHEET.Cells[1, 12] = "ClientDup";
            ...
            foreach (ConversationData c in Trace.conversations)
            {
                ...
                if (showRecord)
                {
                    ...
                    EXCEL_WORKSHEET.Cells[count, 3] = ((c.isIPV6) ? utility.FormatIPV6Address(c.destIPHi, c.destIPLo) : utility.FormatIPV4Address(c.destIP));
                    EXCEL_WORKSHEET.Cells[count, 4] = destPort;
                    EXCEL_WORKSHEET.Cells[count, 5] = ((c.isIPV6) ? "IPV6" : "IPV4");
                    EXCEL_WORKSHEET.Cells[count, 6] = GetProtocolName(c);
                    EXCEL_WORKSHEET.Cells[count, 7] = c.synCount;
                    if (c.synCount > 3)
                    {
                        EXCEL_WORKSHEET.Cells[count, 7].Interior.Color = XlRgbColor.rgbIndianRed;
                        EXCEL_WORKSHEET.Cells[count, 7].Font.Color = XlRgbColor.rgbWhite;

                    }
                    EXCEL_WORKSHEET.Cells[count, 8] = c.finCount;
                    EXCEL_WORKSHEET.Cells[count, 9] = c.resetCount;
                    if (c.resetCount > 0)
                    {
                        EXCEL_WORKSHEET.Cells[count, 9].Interior.Color = XlRgbColor.rgbIndianRed;
                        EXCEL_WORKSHEET.Cells[count, 9].Font.Color = XlRgbColor.rgbWhite;
                    }
            ...
            EXCEL_WORKSHEET.Rows.AutoFit();
            EXCEL_WORKBOOK.SaveAs(EXCEL_OUTPUT_FILEPATH);
        }
