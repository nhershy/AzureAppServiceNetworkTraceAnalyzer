...
namespace AASNTA
{
    ...
    //
    // Usage: AASNTA captureFile [/output outputFile] [[/sql ipaddress,port]...] [/convList] [/filterFmt NETMON|WireShark]
    //

    class Program
    {
        static Application ExcelFile = null;
        static string excelOutputFilePath = string.Empty;
        ...
        static void Main(string[] args)
        {
            ...
            else  // for case-insensitive argument names, use lower-case to match
            {
                ...
                {
                    excelOutputFilePath = ((CommandLineArgs)values[0]).value;
                ...
                {
                    excelOutputFilePath = utility.getExcelFileName(fileSpec);
                    statOutFile = utility.getStatLogFileName(fileSpec);
                    diagOutFile = utility.getDiagLogFileName(statOutFile);
                    outFile = utility.getGenericLogFileName(statOutFile);
                }
            ...
            try
            {
                ...
                Workbook ExcelWorkBook = ExcelFile.Workbooks.Add();
                Worksheet ExcelWorkSheet = ExcelWorkBook.Worksheets[1];

                //statFile = new StreamWriter(statOutFile);
                OutputText.TextReport(Trace, sourceIp, destinationIp, portNumber, ExcelWorkBook, ExcelWorkSheet, excelOutputFilePath);
                //statFile.Close();
                //statFile = null;
            }
            catch (Exception ex)
        ...
        static public void displayUsage()
        {
            ...
            Console.WriteLine();
            Console.WriteLine(@"     AASNTA captureFile [/output outputFile] [[/sql ipaddress,port]...] [/convList] [/filterFmt NETMON|WireShark]");
            Console.WriteLine();
            Console.WriteLine(@"EXAMPLES:");
            Console.WriteLine();
            Console.WriteLine(@"     AASNTA c:\temp\mytrace.cap         ... writes to c:\temp\mytrace.log");
            Console.WriteLine(@"     AASNTA c:\temp\mytrace.pcap        ... writes to c:\temp\mytrace.log");
            Console.WriteLine(@"     AASNTA c:\temp\mytrace.pcapng      ... writes to c:\temp\mytrace.log");
            Console.WriteLine();
            Console.WriteLine(@"     AASNTA c:\temp\mytrace*.cap /output c:\temp\trace.log");
            Console.WriteLine();
            Console.WriteLine(@"     AASNTA c:\temp\mytrace.cap /sql 10.0.0.2,1433     ... hints at SQL Server");
            Console.WriteLine();
