...
namespace AASNTA
{
    ...
    class utility
    {
        ...
        // if the file spec ends with \*.* or \*.cap, make it use AASNTA.log
        public static string getLogFileName(string fileSpec)
        {
            ...
            // check to make sure we have a partial file name ... otherwise, use AASNTA.log
        ...
        public static string getExcelFileName(string LogName)
        {
            string directoryName = Path.GetDirectoryName(LogName);
            string fileName = Path.GetFileNameWithoutExtension(LogName);
            return directoryName + "\\" + fileName + "-results.xlsx";
        }
